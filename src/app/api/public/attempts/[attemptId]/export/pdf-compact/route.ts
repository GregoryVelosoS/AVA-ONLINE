...
function selectedAnswerText(question: {
  ...
  if (question.type === "MULTIPLE_CHOICE") {
    return question.selectedOptionLabel
      ...
      : question.resultStatus === "unanswered" ? "Nao respondida" : "Questao respondida";
  }
  ...
  if (question.type === "FILE_UPLOAD") return "Questao de envio de arquivo.";
  return "Nao respondida";
}
...
export async function GET(_: Request, context: { params: Promise<{ attemptId: string }> }) {
  ...
  writer.heading("Resultado individual da prova (Versao Compacta)", 1);
  ...
  writer.paragraph(`Pontuacao final: ${formatScore(result.totalScore)} / ${formatScore(result.maxScore)} (${result.scorePercent}%)`);
  writer.paragraph(`Totais: ${result.correctCount} acertos · ${result.incorrectCount} erros · ${result.pendingCount} pendentes`);

  writer.divider();
  writer.heading("Gabarito Resumido", 2);
  ...
  for (const question of result.questionResults) {
    ...
    writer.badge(
      ...
      question.resultStatus === "correct" ? "success" : question.resultStatus === "incorrect" ? "error" : question.resultStatus === "pending" ? "warning" : "neutral"
    );
    
    // Mostra apenas o inicio do comando para contextualizar
    const statementSnippet = question.statement.length > 100 ? question.statement.substring(0, 100) + "..." : question.statement;
    writer.paragraph(statementSnippet, { size: 10, color: rgb(0.4, 0.4, 0.4), gapAfter: 6 });

    writer.paragraph(`Sua resposta: ${selectedAnswerText(question)}`);
    writer.paragraph(`Resposta correta: ${question.correctOptionLabel ? `${question.correctOptionLabel}) ${question.correctOptionContent || ""}`.trim() : "Correcao textual/manual"}`);
    
    writer.divider();
  }

  const bytes = await pdf.save();
  const filename = `resultado-compacto-${safeFilenamePart(result.exam.publicCode || result.exam.title)}-${safeFilenamePart(
